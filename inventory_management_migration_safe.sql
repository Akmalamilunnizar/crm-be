-- Inventory Management System Database Migration - SAFE VERSION
-- This script safely creates tables and indexes only if they don't exist
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     ...
     AND INDEX_NAME = 'idx_asset_items_status') = 0,
    'CREATE INDEX idx_asset_items_status ON asset_items (status)',
    'SELECT "Index idx_asset_items_status already exists"'
));
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = 'iqgncnzy_skripsi' 
     AND TABLE_NAME = 'asset_items' 
     AND INDEX_NAME = 'idx_asset_items_created_at') = 0,
    'CREATE INDEX idx_asset_items_created_at ON asset_items (created_at)',
    'SELECT "Index idx_asset_items_created_at already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = 'iqgncnzy_skripsi' 
     AND TABLE_NAME = 'ticket_asset_transactions' 
     AND INDEX_NAME = 'idx_ticket_asset_transactions_type') = 0,
    'CREATE INDEX idx_ticket_asset_transactions_type ON ticket_asset_transactions (transaction_type)',
    'SELECT "Index idx_ticket_asset_transactions_type already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
