-- Apply PSB Views for Installation Reports
USE iqgncnzy_skripsi;

-- Drop existing views if they exist
DROP VIEW IF EXISTS `installation_report_complete`;
DROP VIEW IF EXISTS `installation_summary_per_customer`;

-- Create updated installation_report_complete view with PSB fields
...
-- Create updated installation_summary_per_customer view with PSB fields
CREATE VIEW `installation_summary_per_customer` AS
SELECT 
    c.id as customer_id,
    c.name as customer_name,
    c.address as customer_address,
...
SELECT 'PSB Views applied successfully!' as status;
