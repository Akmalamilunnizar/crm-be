...
Write-Host "=== Document Upload Debug Test ===" -ForegroundColor Green
...
# Step 3: Show curl command with detailed debugging
Write-Host "`nStep 3: Curl command with detailed debugging..." -ForegroundColor Blue
...
curl -X POST "http://localhost:8080/api/admin/customer-installation/report-installations" \
  ...
  -F "installation_notes=Test installation notes"
"@
...
# Step 6: Show troubleshooting steps
Write-Host "`nStep 6: Troubleshooting steps..." -ForegroundColor Blue
Write-Host "If document_photo is still empty:" -ForegroundColor Yellow
Write-Host "1. Check if backend is running: go run cmd/myapp/main.go" -ForegroundColor White
Write-Host "2. Check if file is being uploaded: Look for 'Document photo upload started' log" -ForegroundColor White
Write-Host "3. Check if file is being saved: Look for 'Document photo uploaded successfully' log" -ForegroundColor White
Write-Host "4. Check if file exists: Look in uploads/installations/documents/ directory" -ForegroundColor White
Write-Host "5. Check if path is being set: Look for 'document_photo: uploads/...' in request data log" -ForegroundColor White
Write-Host "6. Check if path is being saved: Look for 'DocumentPhoto: uploads/...' in installation record log" -ForegroundColor White
