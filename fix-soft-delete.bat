@echo off
echo Updating installation_report_complete view to filter soft-deleted records...
echo.

echo Connecting to MySQL and running commands...

mysql -h 103.63.24.139 -u iqgncnzy_skripsi -pXhYJOWlwNgsk iqgncnzy_skripsi -e "
...
if %ERRORLEVEL% EQU 0 (
    echo.
    echo SUCCESS: installation_report_complete view updated successfully!
    echo Installation reports will now only show records where deleted_at IS NULL.
) else (
    echo.
    echo ERROR: Failed to update the view. Please check your MySQL connection and credentials.
)
